...
class HeuristicModerationPredictor:
    ...
    def predict(self, feature_vector: np.ndarray, feature_names: List[str]) -> Dict[str, Any]:
        ...
        if comments_1min > 10:
            ...
            reasoning.append(f"[HIGH RISK] Unusual comment activity ({int(comments_1min)} comments in 1 minute)")
        ...
        if total_comments > 50 and total_views < 100:
            ...
            reasoning.append(f"[HIGH RISK] Suspicious pattern: {int(total_comments)} comments but only {int(total_views)} views")
