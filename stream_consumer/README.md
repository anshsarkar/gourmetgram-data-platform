...
Consumes events from Kafka in real time and maintains rolling aggregations in Redis. When activity on an image crosses a threshold (e.g. goes viral), it publishes a moderation request (inference) to Kafka so the inference service can evaluate it.
